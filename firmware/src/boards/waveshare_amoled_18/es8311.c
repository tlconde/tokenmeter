...
esp_err_t waveshare_i2c_write(uint8_t addr, const uint8_t *data, size_t len);
esp_err_t waveshare_i2c_write_read(uint8_t addr, const uint8_t *write_data,
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return waveshare_i2c_write(es->dev_addr, write_buf, sizeof(write_buf));
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return waveshare_i2c_write_read(es->dev_addr, &reg_addr, 1, reg_value, 1);
}
